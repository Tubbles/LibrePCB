...
bool ShortcutsReferenceGenerator::generatePdf(const FilePath& fp) {
  ...
  int column = 0;
  qreal columnTop = sFirstPageCategoryY;
  x = 0;
  y = columnTop;
  ...
  for (int i = 0; i < categories.count(); ++i) {
    ...
    if ((y + categoryHeight) > sPageHeight) {
      ++column;
      if (column >= sColumnCount) {
        // The last column is full, so continue on a new page. Only the
        // first page carries the logo and the built-in shortcuts.
        drawFooter(writer, painter);
        writer.newPage();
        drawSectionTitle(writer, painter, 0, sPageWidth, sNextPageTitleY,
                         "Configured in Workspace Settings (Continued)");
        column = 0;
        columnTop = sNextPageCategoryY;
      }
      x = column * (sColumnWidth + sColumnSpacing);
      y = columnTop;
      if ((y + categoryHeight) > sPageHeight) {
        // Taller than a whole column, so no pagination can place it.
        layoutOverflow = true;
      }
    }
...
void ShortcutsReferenceGenerator::drawFooter(
    QPdfWriter& writer, QPainter& painter) const noexcept {
