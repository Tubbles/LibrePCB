 ...
 * @date 2017-03-25
 */
class PadSignalMapEditorWidget final : public QWidget, private DevicePadSignalMap::IF_Observer
{
        Q_OBJECT
    ...
    private: // Observer
        void listObjectAdded(const DevicePadSignalMap& list, int newIndex,
                             const std::shared_ptr<DevicePadSignalMapItem>& ptr) noexcept override;
        void listObjectRemoved(const DevicePadSignalMap& list, int oldIndex,
                               const std::shared_ptr<DevicePadSignalMapItem>& ptr) noexcept override;
