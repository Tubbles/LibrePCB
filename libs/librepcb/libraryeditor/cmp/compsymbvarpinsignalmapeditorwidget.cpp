...
CompSymbVarPinSignalMapEditorWidget::CompSymbVarPinSignalMapEditorWidget(QWidget* parent) noexcept :
    QWidget(parent), mTable(new QTableWidget(this)), mWorkspace(nullptr),
    mSignalList(nullptr), mSymbolVariant(nullptr)
{
    mTable->setCornerButtonEnabled(false);
    mTable->setSelectionBehavior(QAbstractItemView::SelectRows);
    mTable->setSelectionMode(QAbstractItemView::SingleSelection);
    mTable->setColumnCount(_COLUMN_COUNT);
    mTable->setHorizontalHeaderItem(COLUMN_SYMBOL,      new QTableWidgetItem(tr("Symbol")));
    mTable->setHorizontalHeaderItem(COLUMN_PIN,         new QTableWidgetItem(tr("Symbol Pin")));
    mTable->setHorizontalHeaderItem(COLUMN_SIGNAL,      new QTableWidgetItem(tr("Component Signal")));
    mTable->setHorizontalHeaderItem(COLUMN_DISPLAYTYPE, new QTableWidgetItem(tr("Designator in Schematics")));
    mTable->horizontalHeader()->setSectionResizeMode(COLUMN_SYMBOL,      QHeaderView::Stretch);
    mTable->horizontalHeader()->setSectionResizeMode(COLUMN_PIN,         QHeaderView::ResizeToContents);
    mTable->horizontalHeader()->setSectionResizeMode(COLUMN_SIGNAL,      QHeaderView::ResizeToContents);
    mTable->horizontalHeader()->setSectionResizeMode(COLUMN_DISPLAYTYPE, QHeaderView::ResizeToContents);
    ...
    connect(mTable, &QTableWidget::currentCellChanged,
            this, &CompSymbVarPinSignalMapEditorWidget::currentCellChanged);

    QVBoxLayout* layout = new QVBoxLayout(this);
    layout->setContentsMargins(0, 0, 0, 0);
    layout->setSpacing(3);
    layout->addWidget(mTable);
    QPushButton* btn = new QPushButton(tr("Automatically assign all signals by name"), this);
    connect(btn, &QPushButton::clicked,
            this, &CompSymbVarPinSignalMapEditorWidget::btnAutoAssignSignalsClicked);
    layout->addWidget(btn);
}

CompSymbVarPinSignalMapEditorWidget::~CompSymbVarPinSignalMapEditorWidget() noexcept
{
}
...
void CompSymbVarPinSignalMapEditorWidget::setVariant(const workspace::Workspace& ws,
    const ComponentSignalList& sigs, ComponentSymbolVariant& variant) noexcept
{
    mWorkspace = &ws;
    mSignalList = &sigs;
...
void CompSymbVarPinSignalMapEditorWidget::currentCellChanged(int currentRow, int currentColumn,
...
void CompSymbVarPinSignalMapEditorWidget::componentSignalChanged(int index) noexcept
{
                                                      ...
                                                      tl::optional<Uuid> selPin) noexcept
{
    ...
    for (int i = 0; i < mSymbolVariant->getSymbolItems().count(); ++i) {
        const ComponentSymbolVariantItem& item = *mSymbolVariant->getSymbolItems().at(i);
...
void CompSymbVarPinSignalMapEditorWidget::setTableRowContent(int row,
    const ComponentSymbolVariantItem& item, const ComponentPinSignalMapItem& map,
    int itemNumber, const Symbol* symbol) noexcept
{
    ...
    displayTypeComboBox->setStyleSheet("padding: 0px 3px 0px 3px;"); // reduce required space
    ...
    connect(displayTypeComboBox, &CmpSigPinDisplayTypeComboBox::currentItemChanged,
            this, &CompSymbVarPinSignalMapEditorWidget::displayTypeChanged);
...
void CompSymbVarPinSignalMapEditorWidget::setDisplayType(const Uuid& item,
    const Uuid& pin, const CmpSigPinDisplayType& dt) noexcept
{
    ComponentSymbolVariantItem& i = *mSymbolVariant->getSymbolItems().get(item);
    ComponentPinSignalMapItem& m =* i.getPinSignalMap().get(pin);
    m.setDisplayType(dt);
...
int CompSymbVarPinSignalMapEditorWidget::getRowOfTableCellWidget(QObject* obj) const noexcept
{
    bool ok = false;
...
const QStringList& CompSymbVarPinSignalMapEditorWidget::getLocaleOrder() const noexcept
{
