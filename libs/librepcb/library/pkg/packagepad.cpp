...
PackagePad::PackagePad(const Uuid& uuid, const ElementName& name) noexcept :
...
PackagePad::PackagePad(const SExpression& node) :
    ...
    mName(node.getValueByPath<ElementName>("name", true))
{
}
...
void PackagePad::setName(const ElementName& name) noexcept
{
    mName = name;
}
