...
CmdSymbolPinEdit::CmdSymbolPinEdit(SymbolPin& pin) noexcept :
    ...
    mOldName(pin.getName()), mNewName(mOldName),
    mOldLength(pin.getLength()), mNewLength(mOldLength),
...
{
    if (!wasEverExecuted()) {
        try {performUndo();} catch (...) { qCritical() << "Undo failed!"; }
    }
}
...
void CmdSymbolPinEdit::setName(const ElementName& name, bool immediate) noexcept
{
